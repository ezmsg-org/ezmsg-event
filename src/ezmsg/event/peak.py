...
class ThresholdCrossingTransformer(
    BaseStatefulTransformer[
        ThresholdSettings, AxisArray, AxisArray, ThresholdCrossingState
    ]
):
    ...
    def _reset_state(self, message: AxisArray) -> None:
        ...
        self._state.data = (
            first_samp if self._state.scaler is None else np.zeros_like(first_samp)
        )
        ...
        self._state.elapsed = np.zeros((np.prod(data.shape[:-1]),), dtype=int) + (
    ...
    def _process(self, message: AxisArray) -> AxisArray:
        ...
        if self._state.data_raw is not None:
            self._state.data_raw = np.concatenate(
                (self._state.data_raw, message.data), axis=-1
            )
        ...
        overs = (
            data >= self.settings.threshold
            if self.settings.threshold >= 0
            else data <= self.settings.threshold
        )
        ...
        if self._state.refrac_width > 2 and len(cross_idx[-1]) > 0:
            ...
            ieis[feat_splits] = (
                cross_idx[-1][feat_splits] + self._state.elapsed[uq_feats]
            )
        ...
        if len(cross_idx[-1]) == 0:
            ...
            self._state.min_width > 1
            or self.settings.align_on_peak
            or self.settings.return_peak_val
        ):
            ...
            ep_overs = overs_padded[
                tuple(_[:, None] for _ in cross_idx[:-1]) + (s_idx,)
            ]
            ...
            if self.settings.align_on_peak or self.settings.return_peak_val:
                ...
                for len_idx, ep_len in enumerate(uq_lens):
                    ...
                    ep_inds_tuple = tuple(_[b_grp, None] for _ in cross_idx[:-1]) + (
                        ep_resamp,
                    )
...
class ThresholdCrossing(
    BaseTransformerUnit[
        ThresholdSettings, AxisArray, AxisArray, ThresholdCrossingTransformer
    ]
):
