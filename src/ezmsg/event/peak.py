...
class ThresholdCrossingTransformer(
    ...
    def _reset_state(self, message: AxisArray) -> None:
        ...
        if self.settings.auto_scale_tau > 0:
            self._state.scaler = AdaptiveStandardScalerTransformer(
                time_constant=self.settings.auto_scale_tau,
                axis="time"
            )
    ...
    def _process(self, message: AxisArray) -> AxisArray:
        ...
        if len(cross_idx[-1]) == 0:
            ...
            result_val = np.ones(
                cross_idx[-1].shape, dtype=data.dtype if self.settings.return_peak_val else bool
            )
            ...
            if self.settings.align_on_peak or self.settings.return_peak_val:
                ...
                if self.settings.return_peak_val:
                    ...
                    peak_inds_tuple = cross_idx if self.settings.align_on_peak else tuple(cross_idx[:-1]) + (cross_idx[-1] + pk_offset,)
...
class ThresholdCrossing(
    BaseTransformerUnit[ThresholdSettings, AxisArray, AxisArray, ThresholdCrossingTransformer]
):
