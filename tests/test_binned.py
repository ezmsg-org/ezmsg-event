...
def test_chunk_invariance_offnominal(fs: float):
    ...
    whole = np.concatenate(
        [
            m.data
            for m in _run(
                BinnedEventAggregator(settings=BinnedEventAggregatorSettings(bin_duration=bin_dur)),
...
def test_fractional_false_sample_locked(fs: float):
    """fractional=False bins a fixed int(bin_duration*fs) sample count, so the
    output gain is int(bin*fs)/fs (sample-locked, matching Window). fs=30030
    (0.02*fs = 600.6) is the discriminating case: truncation gives 600 where
    round() would give 601."""
    bin_dur = 0.02
    n = 300_000
